...
uint32_t OpenFontRender::decodeUTF8(uint8_t *buf, uint16_t *index, uint16_t remaining) {
	...
	// if ((c & 0xF8) == 0xF0) return c;		if (((c & 0xF8) == 0xF0) && (remaining > 3)) {
...
uint16_t OpenFontRender::alphaBlend(uint8_t alpha, uint16_t fgc, uint16_t bgc) {
	...
	uint16_t fgB = ((fgc << 1) & 0x3E) + 1; // Split out and blend 6 bit green channel

	uint32_t xgx = bgc & 0x07E0;
	uint16_t bgR = ((bgc >> 10) & 0x3E) + 1;
	xgx += ((fgc & 0x07E0) - xgx) * alpha >> 8;
	uint16_t bgG = ((bgc >> 4) & 0x7E) + 1; // Recombine channels
